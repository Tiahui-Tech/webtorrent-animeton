...
const dispatchHandlers = {
  ...
  prioritizeTorrent: (infoHash) =>
    controllers.torrentList().prioritizeTorrent(infoHash),
  ...
  selectAudioTrack: (index) =>
    controllers.audioTracks().selectAudioTrack(index),
  toggleAudioTracksMenu: () =>
    controllers.audioTracks().toggleAudioTracksMenu(),
...
function setupIpc() {
  ...
  ipcRenderer.on('wt-file-modtimes', (e, ...args) =>
    tc.torrentFileModtimes(...args)
  );
  ipcRenderer.on('wt-file-saved', (e, ...args) => tc.torrentFileSaved(...args));
  ipcRenderer.on('wt-poster', (e, ...args) => tc.torrentPosterSaved(...args));
  ipcRenderer.on('wt-audio-metadata', (e, ...args) =>
    tc.torrentAudioMetadata(...args)
  );
  ipcRenderer.on('wt-server-running', (e, ...args) =>
    tc.torrentServerRunning(...args)
  );

  ipcRenderer.on('wt-uncaught-error', (e, err) =>
    telemetry.logUncaughtError('webtorrent', err)
  );
...
function resumeTorrents() {
  state.saved.torrents
    ...
    .forEach((s) =>
      controllers.torrentList().startTorrentingSummary(s.torrentKey)
    );
}
